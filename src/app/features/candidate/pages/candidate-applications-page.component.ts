  ...
  template: `
    <section class="space-y-6">
      <div class="bg-white rounded-xl shadow-sm p-5 sm:p-6">
        <h1 class="text-2xl font-bold text-gray-900">{{ 'CANDIDATE.APPLICATIONS.TITLE' | translate }}</h1>
        <p class="mt-2 text-sm text-gray-500">{{ 'CANDIDATE.APPLICATIONS.SUBTITLE' | translate }}</p>
      </div>
      ...
      @if (rows.length) {
        <div class="bg-white rounded-xl shadow-sm overflow-hidden">
          <div class="hidden grid-cols-[minmax(0,2fr)_minmax(0,1.2fr)_160px_160px] gap-4 border-b border-gray-100 px-5 py-3 text-xs font-semibold uppercase tracking-wide text-gray-400 md:grid">
            <span>{{ 'CANDIDATE.APPLICATIONS.JOB_TITLE' | translate }}</span>
            <span>{{ 'CANDIDATE.APPLICATIONS.COMPANY' | translate }}</span>
            <span>{{ 'CANDIDATE.APPLICATIONS.APPLIED_DATE' | translate }}</span>
            <span>{{ 'CANDIDATE.APPLICATIONS.STATUS' | translate }}</span>
          </div>
