  ...
  template: `
    <div class="bg-white rounded-xl shadow-sm p-4 sm:p-6">
      <div class="mb-4 flex items-center justify-between rtl:flex-row-reverse">
        <h2 class="font-bold text-gray-900 rtl:text-right">{{ 'CANDIDATE.PROFILE.RESUME' | translate }}</h2>
        <button *ngIf="resume" type="button" class="text-sm font-medium text-blue-600 hover:text-blue-700">{{ 'CANDIDATE.PROFILE.VIEW' | translate }}</button>
      </div>

      <ng-container *ngIf="resume; else empty">
        <div class="rounded-xl border border-gray-100 bg-gray-50 p-4">
          <p class="text-sm font-semibold text-gray-900">{{ resume.name }}</p>
          <p class="mt-1 text-xs text-gray-400 rtl:text-right">
            {{ 'CANDIDATE.PROFILE.UPLOADED' | translate }} {{ resume.uploadedAt | date:'mediumDate' }} · {{ resume.sizeLabel }}
          </p>
          <p class="hidden mt-1 text-xs text-gray-400 rtl:text-right">
      ...
      <ng-template #empty>
        <p class="mb-4 text-sm text-gray-400 rtl:text-right">{{ 'CANDIDATE.NO_DATA' | translate }}</p>
      </ng-template>
      ...
      >
        {{ 'CANDIDATE.PROFILE.UPLOAD_RESUME' | translate }}
      </button>
    </div>
  `,
})
